                            ...
                            @foreach ($wishlists as $wishlist)
                                <div class="d-sm-flex justify-content-between mt-lg-4 mb-4 pb-3 pb-sm-2 border-bottom">
                                    <div class="d-block d-sm-flex align-items-start text-center text-sm-start">
                                        @php
                                            // Assurez-vous que la fonction renvoie un objet avec la propriété image
                                        ...
                                        <a class="d-block flex-shrink-0 mx-auto me-sm-4" href="{{route("front.product",$wishlist->product->slug)}}" style="width: 10rem;">
                                            @if(!empty($productImage) && file_exists(public_path('uploads/product/small/' . $productImage->image)))
                                                <img class="card-img-top" src="{{ asset('uploads/product/small/' . $productImage->image) }}" alt="{{ $wishlist->product->title }}" />
                                            @else
                                                <!-- Image par défaut -->
                                                <img src="{{ asset('admin-assets/img/default-150x150.png') }}" alt="Image par défaut">
                                            @endif
                                        </a>
                                        ...
                                        <div class="pt-2">
                                            ...
                                            <div class="fs-lg text-accent pt-2">
                                                <span class="h5"><strong>{{ $wishlist->product->price }} DT</strong></span>
                                                @if($wishlist->product->compare_price > 0)
                                                    <span class="h6 text-underline">
                                                        <del>{{ $wishlist->product->compare_price }} DT</del>
                                                    </span>
                                                @endif
                                            </div>
                                        </div>
